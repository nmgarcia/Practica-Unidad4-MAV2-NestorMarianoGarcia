...
void Avatar::SetAwake(bool b) {
	m_pCuerpo->SetAwake(b);
}
...
void Avatar::SetIsDragging(Vector2f position) {
	FloatRect bounds = m_pSprite->getGlobalBounds();
	bool check = bounds.contains(position.x, position.y);
	isDragging = check;
}

void Avatar::SetIsDragging(bool b) {
	isDragging = b;
}

bool Avatar::GetIsDragging() {
	return isDragging;
}
